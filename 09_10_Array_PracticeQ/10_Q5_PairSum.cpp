// Problem Statement:
// You are given an integer array 'ARR' of size 'N' and an integer 'S'. Your task is to return the
// list of all pairs of elements such that each sum of elements of each pair equals 'S'.
...
vector<vector<int>> pairSum(vector<int> &arr, int s)
{
    ...
    sort(ans.begin(), ans.end()); //for given o/p format
...
int main()
{
    ...
    vector<vector<int>> sol = pairSum(arr2, 0);
